  ...
  return (
    <section className="space-y-4">
      <PageHeader
        ...
        description="Recent punch records with computed regular hours, overtime, night differential, late minutes, and undertime."
                ...
                {records.map((record) => (
                  <tr className="transition hover:bg-emerald-50/40" key={record.id}>
                    ...
                    <td className="px-4 py-3 text-emerald-800/70">
                      {record.summary ? `${record.summary.lateMinutes} min` : "-"}
                    </td>
                    <td className="px-4 py-3 text-emerald-800/70">
                      {record.summary ? `${record.summary.undertimeMinutes} min` : "-"}
                    </td>
                  </tr>
                ))}
                {!records.length && (
